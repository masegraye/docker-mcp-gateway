...
**Provisioner Interface**: All provisioners implement a unified interface:
```go
...
**Container Runtime Abstraction**: Provisioners use Container Runtime implementations for actual container operations:
```go
...
**Configuration Resolution**: Just-in-time secret and configuration resolution through ConfigResolver pattern:
```go
...
### Configuration and Secret Provider Architecture

The pluggable provisioning system supports two distinct provider modes for handling configuration templates and secrets, enabling both development flexibility and production security requirements.

#### Docker-Engine Provider Mode
...
- Kubernetes development with dynamic resource creation

#### Cluster Provider Mode
...
- Multi-environment deployments with centralized secret management
- Compliance scenarios requiring audit trails and access controls

#### Provider Mode Selection

**CLI Configuration**:
```bash
# Docker-engine mode (development)
--cluster-config-provider docker-engine --cluster-secret-provider docker-engine

# Cluster mode (production)
--cluster-config-provider cluster --cluster-secret-provider cluster \
--cluster-config-name mcp-config --cluster-secret-name mcp-secrets
```

**Mode-Specific Behavior**:

| Aspect | Docker-Engine Mode | Cluster Mode |
|--------|-------------------|--------------|
| **Secret Resolution** | Docker Desktop credential store | Pre-existing Kubernetes Secrets |
| **Template Processing** | Resolved to actual values | Preserved as secretKeyRef references |
| **Resource Creation** | Dynamic creation during provisioning | Assumes pre-existing resources |
| **Security Model** | Docker Desktop trust model | Kubernetes RBAC + encryption |
| **Operational Model** | Development-focused, automatic | Production-focused, manual pre-provisioning |
...
### 1. Docker Provisioner (`--provisioner=docker`)

**Status**: ✅ Complete - Full backward compatibility with existing functionality

**Capabilities**:
- **Remote HTTP Endpoints**: Direct connection to MCP servers exposing HTTP interfaces
- **Containerized Stdio**: Traditional container-based stdio communication via Docker
- **POCI Tool Support**: Ephemeral containers for tools with native stdout/stderr separation via Docker API
- **Secret Injection**: Just-in-time secret resolution from Docker Desktop credential store
- **Network Management**: Automatic network detection and container attachment

**Container Runtime**: `DockerContainerRuntime` using Docker API for both ephemeral (tools) and persistent (servers) containers

**Configuration Resolution**: Uses Docker Desktop credential store for template resolution (`{{dockerhub.username}}` → actual values)

### 2. Kubernetes Provisioner (`--provisioner=kubernetes`)

**Status**: ✅ Complete - Production-ready with Docker Desktop Kubernetes

**Capabilities**:
- **Pod-Based Deployment**: MCP servers run as Kubernetes pods with full lifecycle management
- **Native Secret Management**: Kubernetes Secrets with `secretKeyRef` environment variable injection
- **ConfigMap Support**: Native Kubernetes ConfigMaps for non-sensitive configuration
- **Session-Based Cleanup**: Automatic pod cleanup on gateway shutdown with session tracking
- **POCI Tool Support**: Ephemeral containers for tools with stdout/stderr separation via sophisticated sidecar pattern

**Container Runtime**: `KubernetesContainerRuntime` using client-go v0.33.4 for Pod management and attach API for stdio streams

**Configuration Resolution**: 
- **Docker-Engine Mode**: Uses Docker Desktop credential store (development)
- **Cluster Mode**: Uses pre-existing Kubernetes ConfigMaps and Secrets (production)
...
### POCI Tool Implementation Details
...
**The Problem**: Kubernetes doesn't natively support stdout/stderr separation for ephemeral containers like Docker does. The Pod logs API always combines streams, and the Pod attach API only works reliably for running containers.
...
```bash
--docker-context CONTEXT    # Specify Docker context
```
...
## Kubernetes Cluster Provider Workflow
...
### 3. Production-Ready Kubernetes Support
...
### 3. Cloud Provisioner Implementation
